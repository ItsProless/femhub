...
export function formatToValidLink(url: string): string {
...
export function DocTOC({
  ...
  const itemIds = React.useMemo(
    () => tocs.map((item) => item.url.replace('#', '')),
    [tocs],
  )
      ...
      <DropdownMenu open={open} onOpenChange={setOpen}>
        ...
        >
          {tocs.map((item) => (
            ...
            >
              <Link href={`#${formatToValidLink(item.url)}`}>{item.title}</Link>
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
  ...
  return (
    <div className={cn('flex flex-col gap-2 p-4 pt-0 text-sm', className)}>
      ...
      {tocs.map((item) => (
        <Link
          key={item.url}
          href={`#${formatToValidLink(item.url)}`}
