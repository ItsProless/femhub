...
import { predicate as commandPredicate, type Command } from '../commands/index.js';
import { predicate as eventPredicate, type Event } from '../events/index.js';
...
export async function loadStructures<Structure>(
	...
	// Create a glob pattern to match the .js/.mjs files
	const basePath = dir instanceof URL ? fileURLToPath(dir) : dir.toString();
	const pattern = resolve(basePath, recursive ? '**/*.{js,mjs}' : '*.{js,mjs}');

	// Loop through all the matching files in the directory
	for await (const file of glob(pattern)) {
		// If the file is index.js or index.mjs, skip the file
		if (
		...
		const { default: structure } = await import(`file://${file}`);
